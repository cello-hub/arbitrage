...
async function updatePairs(network: Network): Promise<ArbitragePair[]> {
  logger.info('Updating arbitrage token pairs')
  ...
  const factories: Contract[] = []

  logger.info(`Fetch from dexes: ${Object.keys(factoryAddrs)}`)
  ...
  for (const key in baseTokens) {
    ...
    for (const quoteKey in quoteTokens) {
      ...
      const tokenPair: TokenPair = {
        symbols: `${quoteToken.symbol}-${baseToken.symbol}`,
        pairs: []
      }
...
function getPairsFile(network: Network): string {
...
export async function tryLoadPairs(network: Network): Promise<ArbitragePair[]> {
  ...
  try {
    ...
    logger.info('Load pairs from json')
  } catch (err) {
    pairs = null
  }
